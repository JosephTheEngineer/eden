...
FileManager::FileManager(){
    ...
    documents = new std::string([[paths objectAtIndex:0] UTF8String]);
	//[documents retain];
    docs=*documents;
    printg("!!!!!! %s\n",documents->c_str());
...
BOOL FileManager::deleteWorld(std::string name){
    NSFileManager* fm=[NSFileManager defaultManager];
    NSString* img_name=[NSString stringWithFormat:@"%s/%s.png",documents->c_str(),name.c_str()];
...
void FileManager::saveWorld(Vector warp){
	...
	[nsstring(*World::getWorld->menu->selected_world->display_name) getCString:sfh->name
														 maxLength:49
...
void FileManager::setImageHash(std::string h){
...
std::string FileManager::getName(std::string n){
...
void FileManager::convertFile(std::string fn){
    NSString* file_name=nsstring(fn);
    NSFileManager* fm=[NSFileManager defaultManager];
    oldFile=[NSFileHandle fileHandleForReadingAtPath:file_name];
    NSString* temp_name=[NSString stringWithFormat:@"%s/temp.map",documents->c_str()];
      ...
      //  clear();
       ...
       g_terrain_type=9;
        ...
        if(sfh->version<1||sfh->version>1000){  //old legacy convert code, no longer really supported
            ...
            convertFile(cpstring(file_name));
