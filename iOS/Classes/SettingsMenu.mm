...
void SettingsMenu::load()
{
    ...
    //World::getWorld->classicMode=properties[S_CLASSIC_MODE].value;
...
void SettingsMenu::save()
{
    ...
    for(int i=0;i<NUM_PROP;i++)
    {
        [prefs setObject:@(properties[i].value) forKey:properties[i].name];
    }
    [prefs synchronize];
    this->load();
}
...
NSString* SettingsMenu::getNewWorldName()
{
    ...
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    [prefs setObject:@(world_counter) forKey:@"new_world_counter"];
    [prefs synchronize];
