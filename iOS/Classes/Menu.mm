...
void Menu::loadWorlds()
{
  ...
  /*  for(int i=0;i<dirc;i++){
    ...
    for(int i=0;i<dirc;i++)
    {
        NSString* file_name=dirContents[i];
        NSLog(@"%@",file_name);
       ...
       // file_name=[file_name stringByDeletingPathExtension];
        ...
        node->display_name=new std::string(cpstring(real_name));
    ...
    for(int i=0;i<MAX_TOUCHES;i++)
    {
        ...
        if(touches[i].inuse==usage_id&&touches[i].down==M_RELEASE)
        {
            ...
            if(inbox2(touches[i].mx,touches[i].my,&rect_create))
            {
                ...
                addWorld(new_world);
                selected_world=new_world;                
                sbar->setStatus([NSString stringWithFormat:@"%s created",new_world->display_name->c_str()]
                               ,2);
                fnbar->setStatus(nsstring(*selected_world->display_name) ,9999);
            }
