...
BOOL collidePolyhedra(Polyhedra A,Polyhedra B)
{
    ...
    for(int i=0;i<A.n_faces;i++)
    {
        ...
        if (AxisSeparatePolygons(A.faces[i].normal, A.points,A.n_points, B.points,B.n_points))
            return false;
    ...
    for(int i=0;i<B.n_faces;i++)
    {
       ...
       // Vector positionedNormal=v_add(B.faces[i].normal,B.faces[i].points[0]);
        if (AxisSeparatePolygons(B.faces[i].normal, A.points,A.n_points, B.points,B.n_points))
            return false;
