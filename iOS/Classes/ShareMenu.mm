...
ShareMenu::ShareMenu(){
	...
	label_bar->setStatus(@"Name your world: ",9999,NSTextAlignmentLeft);
...
void ShareMenu::endShare(BOOL cancel){
    ...
    NSString* image_file_name=[NSString stringWithFormat:@"%s/%s.png",
                               World::getWorld->fm->documents->c_str(),node->file_name->c_str()];
...
void ShareMenu::update(float etime){
    ...
    if(cursor_blink<-.3f){
        ...
        name_bar->setStatus([NSString stringWithFormat:@"%@|",displays],9999,NSTextAlignmentLeft);
     }
